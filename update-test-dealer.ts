...
async function main() {
  // Önce mevcut bayiyi bulalım
  const existing = await prisma.customer.findUnique({
    where: { dealerCode: 'NX0001' }
  });

  if (existing) {
    // Mevcut bayinin şifresini resetleyelim
    ...
    console.log('Mevcut Bayi Güncellendi:');
    console.log('─────────────────────────────────────────────────────────────');
    console.log(`Bayi Kodu: ${updated.dealerCode}`);
    console.log(`Firma: ${updated.companyName}`);
    console.log(`Yeni Şifre: ${password}`);
    console.log(`Email: ${updated.email}`);
    console.log(`Tel: ${updated.phone}`);
    console.log('');
    console.log('Login bilgileri:');
    console.log(`  Dealer Code: ${updated.dealerCode}`);
