...
function HeroBanner({ total, onSearch }: { total: number; onSearch: (q: string) => void }) {
  ...
  return (
    <section className="relative bg-white border-b border-[#e9e9e9] overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#f9f9f9] via-white to-white" />

      <div className="relative max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
        <div className="max-w-2xl">
          <div className="flex items-center gap-2 mb-6">
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-[#767676] uppercase tracking-widest">
              <Zap className="h-3.5 w-3.5" aria-hidden />
              5.000+ Ürün
            </span>
            <span className="w-px h-4 bg-[#e9e9e9]" />
            <span className="text-xs font-medium text-[#767676] uppercase tracking-widest">
          ...
          <form onSubmit={handleSubmit} className="flex max-w-xl" role="search" aria-label="Ürün ara">
            <div className="flex flex-1 h-14 bg-[#f9f9f9] border border-[#e9e9e9] overflow-hidden focus-within:border-[#999] focus-within:ring-4 focus-within:ring-[#f3f3f3]/50 transition-all rounded-l-[20px] rounded-r-lg">
              <input
                type="search"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Ürün, marka veya model ara..."
                className="flex-1 px-6 text-[15px] text-[#1e1e1e] placeholder:text-[#999] focus:outline-none bg-transparent"
        ...
        <div className="hidden lg:flex items-center gap-8 mt-12 pt-8 border-t border-[#e9e9e9]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#f3f3f3] flex items-center justify-center">
              <Shield className="h-5 w-5 text-[#1e1e1e]" aria-hidden />
            </div>
            <div>
              <p className="text-sm font-semibold text-[#1e1e1e]">2 Yıl Garanti</p>
              <p className="text-xs text-[#767676]">Tüm ürünlerde</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#f3f3f3] flex items-center justify-center">
              <Truck className="h-5 w-5 text-[#1e1e1e]" aria-hidden />
            </div>
            <div>
              <p className="text-sm font-semibold text-[#1e1e1e]">Hızlı Teslimat</p>
              <p className="text-xs text-[#767676]">24-48 saat içinde</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#f3f3f3] flex items-center justify-center">
              <Headphones className="h-5 w-5 text-[#1e1e1e]" aria-hidden />
            </div>
            <div>
              <p className="text-sm font-semibold text-[#1e1e1e]">7/24 Destek</p>
              <p className="text-xs text-[#767676]">WhatsApp ile</p>
            </div>
          </div>
        </div>
      </div>
...
function EmptyState({ onClear }: { onClear: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center gap-5">
      <div className="w-24 h-24 rounded-[20px] bg-[#f3f3f3] flex items-center justify-center">
        <Package className="h-10 w-10 text-[#bebebe]" aria-hidden />
      </div>
      <div className="space-y-2">
        <p className="font-semibold text-xl text-[#1e1e1e]">Ürün Bulunamadı</p>
...
function MobileFilterSheet({
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-40 bg-[#1e1e1e]/50 backdrop-blur-sm"
        onClick={onClose}
        aria-hidden
      />
      <div
      ...
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#e9e9e9]">
          <p className="font-semibold text-base text-[#1e1e1e]">Filtreler</p>
          <button
            type="button"
            onClick={onClose}
            className="p-2 text-[#999] hover:text-[#1e1e1e] hover:bg-[#f3f3f3] rounded-lg transition-colors"
    ...
    return (
      <div>
        ...
        {hasChildren && isExpanded && (
          <div className="ml-3 border-l-2 border-[#e9e9e9] pl-1 mt-0.5 mb-0.5 space-y-0.5">
      ...
      <div className="flex items-center justify-between">
        ...
        {filters.categorySlug || filters.brandSlug || filters.search ? (
          <button
            ...
            className="text-xs font-medium text-[#767676] hover:text-[#1e1e1e] transition-colors"
          >
            Temizle
          </button>
        ) : null}
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-4 text-sm bg-[#f9f9f9] border border-[#e9e9e9] rounded-lg focus:outline-none focus:border-[#999] focus:ring-2 focus:ring-[#f3f3f3] transition-all"
            ...
            {visibleBrands.map((brand) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 py-2 px-3 cursor-pointer rounded-lg hover:bg-[#f9f9f9] transition-colors w-full text-left"
          ...
          {filteredBrands.length > 8 && (
            <button
              ...
              className="flex items-center gap-1 text-xs font-medium text-[#767676] hover:text-[#1e1e1e] transition-colors"
            >
    ...
    <div className="flex items-center gap-3 flex-wrap">
      <span className="text-sm text-[#767676]">
        {isLoading ? (
          ...
          <>
            <span className="font-semibold text-[#1e1e1e]">{total.toLocaleString("tr-TR")}</span> ürün
          </>
        )}
      </span>
      ...
      {activeTags.map((tag) => (
        <span
          key={tag.key}
          className="inline-flex items-center gap-1.5 h-8 px-3 bg-[#f3f3f3] text-[#555] text-sm font-medium rounded-full"
        >
          {tag.label}
          <button
            type="button"
      ...
      {activeTags.length > 1 && (
        <button
          type="button"
          onClick={onClearAll}
          className="text-sm text-[#767676] hover:text-red-500 transition-colors"
        >
          Tümünü temizle
        </button>
      )}
            ...
            <div className="bg-white rounded-[20px] p-4 shadow-sm">
              <div className="flex items-center gap-4 flex-wrap">
                <button
                  ...
                  className="lg:hidden inline-flex items-center gap-2 h-10 px-4 bg-[#f9f9f9] border border-[#e9e9e9] text-sm font-medium text-[#555] hover:bg-[#f3f3f3] transition-colors rounded-lg"
                ...
                <div className="flex items-center gap-3 ml-auto">
                  ...
                  >
                    <SelectTrigger
                      size="sm"
                      className="w-40 h-10 border-[#e9e9e9] text-sm font-medium rounded-lg focus:ring-0"
                      aria-label="Sıralama"
                    >
                      <SelectValue />
                    </SelectTrigger>
                  ...
                  <div
                    className="flex bg-[#f3f3f3] rounded-lg overflow-hidden p-1"
                    role="group"
