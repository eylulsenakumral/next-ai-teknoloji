...
export function PublicHeaderNext() {
  ...
  const headerClass = cn(
    ...
    home
      ? "border-white/10 bg-[#071426]/90 text-white"
      : "border-slate-200 bg-white/95 text-[#071426]"
  )
    ...
    <header className={headerClass}>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-5">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isCta
                    ? "rounded-lg border border-[#1477ff]/40 bg-[#1477ff]/10 px-3 py-1.5 text-[#1477ff] hover:bg-[#1477ff]/20 opacity-100"
                    : isActive
                      ? "text-[#58a3ff]"
                      : "text-current opacity-75 hover:text-[#1477ff] hover:opacity-100"
                )}
              >
                {label}
              </Link>
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/bayimiz-olun"
            className="hidden text-xs font-bold opacity-70 transition hover:text-[#1477ff] hover:opacity-100 xl:block"
          >
            Bayimiz Olun
          </Link>
          <Link
            href="/bayi-giris"
            className="hidden rounded-lg bg-[#1477ff] px-3 py-2 text-xs font-bold text-white transition hover:bg-[#2d8cff] sm:block"
          >
            Bayi Girişi
          </Link>
          <Link
            href="/teklif-iste"
            className="rounded-lg bg-[#ff9b43] px-3 py-2 text-xs font-bold leading-5 text-[#071426] transition hover:bg-[#ffad64] md:px-4"
          >
            Teklif İste
          </Link>
        ...
        <div className="lg:hidden mt-4 pb-2 space-y-1">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-[#1477ff]/10 text-[#1477ff]"
                    : home
                      ? "text-white/80 hover:bg-white/5"
                      : "text-[#071426]/80 hover:bg-slate-100"
                )}
              >
                {label}
              </Link>
          ...
          <div className="pt-2 mt-2 border-t border-current/10 flex gap-2">
            <Link
              href="/bayi-giris"
              onClick={() => setMobileOpen(false)}
              className="flex-1 text-center rounded-lg bg-[#1477ff] px-3 py-2 text-xs font-bold text-white"
            >
              Bayi Girişi
            </Link>
            <Link
