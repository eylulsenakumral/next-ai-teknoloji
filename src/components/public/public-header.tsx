  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 left-0 z-50 w-[320px] max-w-[85vw] bg-white shadow-2xl flex flex-col animate-[slideInLeft_0.3s_ease-out]">
        {/* Drawer header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--color-border)] bg-[var(--color-primary)]">
        ...
        <div className="flex-1 overflow-y-auto">
          <nav className="px-3 py-4" aria-label="Mobil navigasyon">
            <Link
              href="/"
              onClick={onClose}
              className="flex items-center gap-3 px-4 py-3 text-[14px] font-medium text-[var(--color-primary)] hover:bg-[var(--color-background)] hover:text-[var(--color-primary)] transition-colors rounded-lg"
            >
              Ana Sayfa
            </Link>
            <Link
              ...
              className="flex items-center gap-3 px-4 py-3 text-[14px] font-medium text-[var(--color-primary)] hover:bg-[var(--color-background)] hover:text-[var(--color-primary)] transition-colors rounded-lg"
            >
              <Package className="h-4 w-4 text-[var(--color-primary)]" aria-hidden />
              Tüm Ürünler
            </Link>
            <Link
              href="/katalog"
          ...
          <div className="px-3 py-3">
            <p className="px-4 text-[11px] font-semibold text-[var(--color-text-muted)] uppercase tracking-widest mb-1">
              Hesap
            </p>
              ...
              {isAuthenticated ? (
                <>
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-3 text-[14px] font-medium text-[var(--color-primary)] hover:bg-[var(--color-background)] hover:text-[var(--color-primary)] transition-colors rounded-lg"
                  >
                    <User className="h-4 w-4 text-[var(--color-primary)]" aria-hidden />
                    {userName || "Hesabım"}
                  </Link>
                  <button
                    type="button"
                ...
                <>
                  ...
                  <Link
                    ...
                    className="flex items-center gap-3 px-4 py-3 text-[14px] font-medium text-[var(--color-primary)] hover:bg-[var(--color-background)] hover:text-[var(--color-primary)] transition-colors rounded-lg"
                  >
                    <LogIn className="h-4 w-4 text-[var(--color-primary)]" aria-hidden />
                    Hesap Oluştur
                  </Link>
                </>
              )}
  ...
  return (
    <div className="flex-1 max-w-2xl mx-8 relative" ref={dropdownRef}>
      ...
      >
        <div className="relative flex items-center">
          <input
            ...
            className="w-full h-[46px] pl-5 pr-14 bg-[var(--color-background)] rounded-[20px] border border-transparent text-[14px] text-[var(--color-primary)] placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-primary)] focus:bg-white outline-none transition-all duration-300"
            aria-label="Arama"
      ...
      {isOpen && (
        <div
          id="search-results"
          className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-[var(--color-border)] max-h-[400px] overflow-y-auto z-50"
          role="listbox"
        >
          {isLoading ? (
            ...
            <>
                ...
                {results.map((product) => (
                  <li key={product.id} role="option">
                    <button
                      ...
                      className="w-full px-4 py-2.5 flex items-center gap-3 hover:bg-[var(--color-background)] transition-colors text-left"
                      role="option"
                      aria-selected="false"
                    >
                      {/* Product Image */}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] font-semibold text-[var(--color-primary)] truncate">
                          {product.name}
                        </p>
              ...
              <div className="border-t border-[var(--color-border)] px-4 py-2">
                <Link
                  ...
                  className="block text-center text-[13px] font-semibold text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors py-1"
                >
                  Tümünü Gör →
                </Link>
              </div>
            ...
            <div className="px-4 py-6 text-center">
              <p className="text-[13px] text-[var(--color-text-muted)]">Sonuç bulunamadı</p>
    ...
    <>
      ...
      <header
        className={cn(
          ...
          isSticky
            ? "sticky top-0 shadow-md bg-white/80 backdrop-blur-md border-[var(--color-border)]/50"
          ...
          <div className="flex items-center justify-between gap-6 h-[72px]">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="lg:hidden flex h-10 w-10 items-center justify-center text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors"
            ...
            <div className="flex items-center gap-3">
              ...
              {isAuthenticated && user?.companyName ? (
                <Link
                  href="/hesabim"
                  className="hidden lg:flex items-center gap-1.5 text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors text-sm font-medium nav-underline"
              ...
              <Link
                href="/katalog"
                className="hidden sm:flex items-center gap-1 text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors relative nav-underline"
              ...
              <Link
                href="/sepet"
                className="relative flex items-center gap-1 text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors"
