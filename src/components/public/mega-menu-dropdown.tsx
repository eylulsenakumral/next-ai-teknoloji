      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[1200px] max-h-[80vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100">
            <h2 className="text-lg font-bold text-[#1e1e1e]">Tüm Kategoriler</h2>
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-lg text-gray-400 hover:text-[#1e1e1e] hover:bg-gray-100 transition-colors"
              aria-label="Kapat"
            >
              <X className="h-5 w-5" />
            </button>
                ...
                {categories.map((cat) => (
                  <div key={cat.id}>
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 text-[13px] font-bold text-[#1e1e1e] uppercase tracking-wide hover:text-[#0040a4] transition-colors"
                    >
                      {cat.name}
                      {cat.productCount ? (
