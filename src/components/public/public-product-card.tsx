        ...
        {product.stockStatus && (
          <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-10">
            <button
              type="button"
              onClick={() => router.push(`/katalog/${product.slug}`)}
              className="w-full py-3 bg-[#0040a4] text-white font-semibold rounded-lg hover:bg-[#15294a] transition-colors flex items-center justify-center gap-2"
            >
              <Eye className="w-5 h-5" aria-hidden />
              Ürünü İncele
            </button>
          ...
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
            ...
            <button
              ...
              className={cn(
                ...
                justAdded ? "bg-[#3b7300] text-white" : "bg-[#0040a4] text-white hover:bg-[#15294a]"
              )}
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center gap-4 p-4 sm:p-5">
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          <Link
            href={`/katalog/${product.slug}`}
            className="block text-[15px] font-semibold text-[#1e3a5f] hover:text-[#0040a4] transition-colors leading-snug line-clamp-2"
          >
            {product.name}
          </Link>
          {product.brand && (
